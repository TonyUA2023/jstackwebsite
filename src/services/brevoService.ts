...
export async function sendLeadToBrevo(data: LeadFormData): Promise<{ success: boolean; message: string }> {
  try {
    ...
    if (!brevoApiKey) {
      console.warn("VITE_BREVO_API_KEY is not defined. Fallback to direct client mail.");
      const mailtoSubject = encodeURIComponent(`⚡ NUEVO PROSPECTO JSTACK: ${data.name} (${data.companyName || 'Empresa'})`);
      const mailtoBody = encodeURIComponent(`Nombre: ${data.name}\nEmpresa: ${data.companyName || 'N/A'}\nEmail: ${data.email}\nTel: ${data.phone || 'N/A'}\nConsulta/Proyecto: ${data.message || ''}`);
      window.open(`mailto:jstackinfo@gmail.com?subject=${mailtoSubject}&body=${mailtoBody}`, '_blank');
      return { success: true, message: "Información procesada y enviada a JSTACK." };
    }
    ...
    const response = await fetch('https://api.brevo.com/v3/smtp/email', {
      ...
      body: JSON.stringify({
        ...
        subject: `⚡ NUEVO PROSPECTO WEB: ${data.name} - ${data.companyName || 'Empresa'}`,
        htmlContent: `
          <div style="font-family: Arial, sans-serif; padding: 25px; color: #0F172A; max-width: 600px; border: 1px solid #CBD5E1; border-top: 4px solid #D8202A; background-color: #FFFFFF;">
            <h2 style="color: #D8202A; border-bottom: 2px solid #E2E8F0; padding-bottom: 10px; margin-top: 0;">⚡ Nuevo Prospecto Web (JSTACK Digital Solutions)</h2>
            
            <table style="width: 100%; border-collapse: collapse; margin-top: 15px;">
              <tr>
                <td style="padding: 8px 0; font-weight: bold; width: 140px; color: #475569;">Nombre Completo:</td>
              ...
              <tr>
                ...
                <td style="padding: 8px 0; color: #0F172A; font-weight: bold;">${data.companyName || 'No especificada'}</td>
              </tr>
              <tr>
                <td style="padding: 8px 0; font-weight: bold; color: #475569;">Email Directo:</td>
                <td style="padding: 8px 0;"><a href="mailto:${data.email}" style="color: #D8202A; font-weight: bold;">${data.email}</a></td>
              </tr>
              <tr>
            ...
            <div style="margin-top: 20px; padding: 15px; background-color: #F8FAFC; border-left: 4px solid #D8202A; border: 1px solid #E2E8F0;">
              <p style="margin: 0; font-weight: bold; color: #334155; margin-bottom: 5px;">Descripción del Proyecto / Consulta:</p>
    ...
    if (response.ok) {
      return { success: true, message: "¡Mensaje recibido! Nos pondremos en contacto contigo en breve." };
    } else {
